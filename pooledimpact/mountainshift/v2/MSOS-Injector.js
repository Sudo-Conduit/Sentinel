// Single inline IIFE - creates worker AND injects code
(function()
{
    'use strict';
 ...
 * CPU.js — UMD + IIFE
 *
 * Self-executing. Returns CPU constructor.
...
(function(root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    if (typeof global !== 'undefined') {
      global.CPU = module.exports;
    }
  } else {
    // Browser / Worker / globalThis
    var CPU = factory();
    root.CPU = CPU;
  ...
  function CPU(options) {
    ...
    // MSRs
    this.MSR = {};
    this.MSR['0x1A0'] = 0x00000000;
    this.MSR['0x2FF'] = 0x00000000;

    // Memory
    var memSize = options.memorySize || 0x100000;
    ...
    this.PIC = {
      master: {
        ...
        ISR: 0x0000,
        baseVector: 0x20,
        icw: { icw1: 0x00, icw2: 0x20, icw3: 0x04, icw4: 0x01 }
      },
      slave: {
        IMR: 0xFFFF,
        IRR: 0x0000,
        ISR: 0x0000,
        baseVector: 0x28,
        icw: { icw1: 0x00, icw2: 0x28, icw3: 0x02, icw4: 0x01 }
      }
    };
  ...
  // ─── PROTOTYPE METHODS ──────────────────────────────────────────
  CPU.prototype.registerInstructions = function() {
    var self = this;

    // MOV
    this.instructions['MOV'] = function(dest, src) {
      if (dest.charAt(0) === 'E') {
        if (typeof src === 'string' && src.charAt(0) === 'E') {
          self[dest] = self[src];
        } else if (typeof src === 'string' && src.indexOf('0x') === 0) {
          self[dest] = parseInt(src, 16);
        } else {
          self[dest] = src;
        }
      } else if (dest.charAt(0) === '[' && dest.charAt(dest.length - 1) === ']') {
        var addr = self.getAddress(dest);
        if (typeof src === 'string' && src.charAt(0) === 'E') {
          self.memoryWrite(addr, self[src]);
        } else {
          self.memoryWrite(addr, src);
        }
      }
      self.EIP += 3;
    };
    ...
    this.instructions['ADD'] = function(dest, src) {
      ...
      if (typeof src === 'string' && src.charAt(0) === 'E') {
        ...
        value = parseInt(src, 16);
      }
      self[dest] += value;
      self.updateFlags(self[dest]);
      self.EIP += 3;
    };
    ...
    this.instructions['PUSH'] = function(src) {
      ...
      self.ESP -= 4;
      self.memoryWrite(self.ESP, value);
      self.EIP += 2;
    };

    // POP
    this.instructions['POP'] = function(dest) {
      var value = self.memoryRead(self.ESP);
      self[dest] = value;
      self.ESP += 4;
      self.EIP += 2;
    };
    ...
    this.instructions['JMP'] = function(addr) {
      if (typeof addr === 'string' && addr.indexOf('0x') === 0) {
        self.EIP = parseInt(addr, 16);
      } else if (typeof addr === 'string' && addr.charAt(0) === 'E') {
        self.EIP = self[addr];
      } else {
        self.EIP = addr;
      }
    ...
    this.instructions['CALL'] = function(addr) {
      self.ESP -= 4;
      self.memoryWrite(self.ESP, self.EIP + 3);
      if (typeof addr === 'string' && addr.indexOf('0x') === 0) {
        self.EIP = parseInt(addr, 16);
      } else if (typeof addr === 'string' && addr.charAt(0) === 'E') {
        self.EIP = self[addr];
    ...
    // RET
    this.instructions['RET'] = function() {
      self.EIP = self.memoryRead(self.ESP);
      self.ESP += 4;
    };
    ...
    this.instructions['CMP'] = function(dest, src) {
      ...
      if (typeof src === 'string' && src.charAt(0) === 'E') {
        ...
        srcValue = parseInt(src, 16);
      }
      var result = self[dest] - srcValue;
    ...
    this.instructions['JE'] = function(addr) {
      if (self.EFLAGS & 0x40) {
        self.EIP = typeof addr === 'string' && addr.indexOf('0x') === 0 ? parseInt(addr, 16) : addr;
      } else {
        self.EIP += 3;
      }
    };
    ...
    this.instructions['INT'] = function(num) {
      var intNum = typeof num === 'string' && num.indexOf('0x') === 0 ? parseInt(num, 16) : parseInt(num, 10);
      self.triggerInterrupt(intNum);
      self.EIP += 2;
    };

    // STI
    this.instructions['STI'] = function() {
      self.EFLAGS |= 0x200;
    ...
    // CLI
    this.instructions['CLI'] = function() {
      self.EFLAGS &= ~0x200;
    ...
    // HLT
    this.instructions['HLT'] = function() {
    ...
    this.instructions['IN'] = function(dest, port) {
      var portNum = typeof port === 'string' && port.indexOf('0x') === 0 ? parseInt(port, 16) : parseInt(port, 10);
      self[dest] = self.IO.ports[portNum] || 0x00;
      self.EIP += 3;
    };
    ...
    this.instructions['OUT'] = function(port, src) {
      ...
      self.IO.ports[portNum] = value;
      self.EIP += 3;
    };

    // RDMSR
    this.instructions['RDMSR'] = function(msr) {
      var msrNum = typeof msr === 'string' && msr.indexOf('0x') === 0 ? parseInt(msr, 16) : parseInt(msr, 10);
      var key = '0x' + msrNum.toString(16);
      var value = self.MSR[key] || 0x00000000;
      self.EAX = value & 0xFFFFFFFF;
      self.EDX = (value >> 32) & 0xFFFFFFFF;
      self.EIP += 3;
    };
    ...
    this.instructions['WRMSR'] = function(msr) {
      var msrNum = typeof msr === 'string' && msr.indexOf('0x') === 0 ? parseInt(msr, 16) : parseInt(msr, 10);
      var key = '0x' + msrNum.toString(16);
      var value = (self.EDX << 32) | self.EAX;
      self.MSR[key] = value;
      self.EIP += 3;
    };

    // INVD
    this.instructions['INVD'] = function() {
      self.config.caching = false;
      self.EIP += 1;
    };

    // WBINVD
    this.instructions['WBINVD'] = function() {
      self.config.caching = false;
      self.EIP += 1;
    };
    ...
    this.instructions['LIDT'] = function(addr) {
      var base = self.memoryRead(addr);
      var limit = self.memoryRead(addr + 2);
      self.IDTR.base = base;
    ...
    // LGDT
    this.instructions['LGDT'] = function(addr) {
      self.EIP += 3;
    };
  };

  // ─── MEMORY OPERATIONS ──────────────────────────────────────────
  CPU.prototype.memoryRead = function(addr) {
    var byte0 = this.memory[addr] || 0;
    var byte1 = this.memory[addr + 1] || 0;
    var byte2 = this.memory[addr + 2] || 0;
    var byte3 = this.memory[addr + 3] || 0;
    return (byte0 << 0) | (byte1 << 8) | (byte2 << 16) | (byte3 << 24);
  };

  CPU.prototype.memoryWrite = function(addr, value) {
  ...
  CPU.prototype.getAddress = function(expr) {
    var clean = expr.replace('[', '').replace(']', '');
    if (clean.charAt(0) === 'E') {
      return this[clean];
    } else if (clean.indexOf('0x') === 0) {
  ...
  // ─── FLAGS ──────────────────────────────────────────────────────
  CPU.prototype.updateFlags = function(value) {
    this.EFLAGS = 0;
  ...
  CPU.prototype.triggerInterrupt = function(intNum) {
    this.ESP -= 4;
    this.memoryWrite(this.ESP, this.EFLAGS);
    this.ESP -= 4;
    this.memoryWrite(this.ESP, this.CS);
    this.ESP -= 4;
    this.memoryWrite(this.ESP, this.EIP);

    var handler = this.IDT[intNum];
    if (handler) {
      this.CS = handler.selector;
      this.EIP = handler.offset;
    } else {
      this.EIP = 0x0000F000;
    }
  };
  ...
  // ─── CACHE ──────────────────────────────────────────────────────
  CPU.prototype.enableCache = function() {
    var misc = this.MSR['0x1A0'] || 0;
    this.MSR['0x1A0'] = misc | 0x400000;
  ...
  CPU.prototype.disableCache = function() {
    this.config.caching = false;
  };

  // ─── PROTECTED MODE ────────────────────────────────────────────
  CPU.prototype.enableProtectedMode = function() {
    this.CR0 |= 0x1;
    this.config.protectedMode = true;
    this.config.realMode = false;
  };

  CPU.prototype.enablePaging = function() {
    this.CR0 |= 0x80000000;
  ...
  CPU.prototype.fetch = function() {
    return this.memoryRead(this.EIP);
  };

  CPU.prototype.decode = function(opcode) {
    for (var name in this.instructions) {
      if (this.instructions.hasOwnProperty(name)) {
        if (opcode === this.hashInstruction(name)) {
  ...
  CPU.prototype.hashInstruction = function(name) {
    var hash = 0;
  ...
  CPU.prototype.execute = function(instruction, args) {
    if (this.instructions[instruction]) {
      this.instructions[instruction].apply(this, args);
      return true;
    }
    return false;
  };
  ...
  // ─── RUN ────────────────────────────────────────────────────────
  CPU.prototype.run = function(steps) {
    steps = steps || 0;
    var count = 0;
    while (!this.config.halted && (steps === 0 || count < steps)) {
      ...
      var args = this.parseArgs(this.EIP + 1);
  ...
  CPU.prototype.parseArgs = function(eip) {
    var args = [];
    var arg1 = this.memoryRead(eip);
    var arg2 = this.memoryRead(eip + 4);
    args.push(arg1);
    args.push(arg2);
    return args;
  };
  ...
  // ─── SUBSTRATE ──────────────────────────────────────────────────
  CPU.prototype.setSubstrate = function(substrate) {
    this.substrate = substrate;
  };

  CPU.prototype.getSubstrate = function() {
  ...
  CPU.prototype.dumpState = function() {
    return {
      registers: {
        EAX: this._toHex32(this.EAX),
        EBX: this._toHex32(this.EBX),
        ECX: this._toHex32(this.ECX),
        EDX: this._toHex32(this.EDX),
        ESI: this._toHex32(this.ESI),
        EDI: this._toHex32(this.EDI),
        EBP: this._toHex32(this.EBP),
        ESP: this._toHex32(this.ESP),
        EIP: this._toHex32(this.EIP),
        EFLAGS: this._toHex32(this.EFLAGS)
      },
      segments: {
        CS: this._toHex16(this.CS),
        DS: this._toHex16(this.DS),
        SS: this._toHex16(this.SS),
        ES: this._toHex16(this.ES),
        FS: this._toHex16(this.FS),
        GS: this._toHex16(this.GS)
      },
      control: {
        CR0: this._toHex32(this.CR0),
        CR2: this._toHex32(this.CR2),
        CR3: this._toHex32(this.CR3),
        CR4: this._toHex32(this.CR4)
      },
      config: this.config,
      halted: this.config.halted || false,
      memoryUsed: this.memory.length
    };
  };

  CPU.prototype._toHex32 = function(value) {
    var hex = value.toString(16);
    while (hex.length < 8) hex = '0' + hex;
    return hex;
  };

  CPU.prototype._toHex16 = function(value) {
    var hex = value.toString(16);
  ...
  CPU.prototype.boot = function() {
    ...
    this.CR3 = 0x00000000;
    this.CR4 = 0x00000000;
    this.config.realMode = true;
    this.config.protectedMode = false;
    this.config.longMode = false;
    this.config.paging = false;
    this.config.caching = false;
    this.config.interrupts = false;
...
var strPadding = String(" ").repeat(4);


console.log('[HOST] BIOS Starting');

console.log(strPadding + '[CPU] Powering On');
var cpu = new CPU({ memorySize: 0x100000 });

console.log(strPadding + '[CPU] Version');
console.log(strPadding + 'CPU Version: v' + CPU.version);
cpu.boot();

console.log(strPadding + '[CPU] Cache enabled');
cpu.enableCache();

console.log(strPadding + '[CPU] Load BIOS Code');
cpu.execute('STI', []);

// Load BIOS code
var tmstart = Date.now();
var bios = [
    0xBC, 0x00, 0x7C, // MOV ESP, 0x7C00
    0xB8, 0x00, 0x00, // MOV EAX, 0x0000
    0x8E, 0xD8,       // MOV DS, AX
    0x8E, 0xC0,       // MOV ES, AX
    0x8E, 0xD0,       // MOV SS, AX
    0xB8, 0x01, 0x00, // MOV EAX, 0x0001
    0xCD, 0x10,       // INT 0x10 (video)
    0xF4              // HLT
  ];

  for (var i = 0; i < bios.length; i++) {
    cpu.memory[0x7C00 + i] = bios[i];
  }
var tmend = Date.now();var totaltime=tmend - tmstart;
console.log(strPadding + '[CPU] BIOS Code Loaded - '+ totaltime + 'ms');

console.log(strPadding + '[CPU] SET EIP: 0x7C00');
cpu.EIP = 0x7C00;

console.log(strPadding + '[CPU] RUN BIOS - START');
//cpu.run();
console.log(strPadding + '[CPU] RUN BIOS - END');
 ...
 *
 * @version 2.2.05 (enableDebugger/enableDiff flags + content-hash caching)
 * @author Will Fobbs III
 * @license MIT
 * @created 2026-07-06T17:57:15Z
 * @modified 2026-07-14T11:33:12Z
 *
...
}(typeof self !== 'undefined' ? self : this, function ()
{
    ...
    class BaseClassX
    {
        ...
        linkTo(target, label = null, options = {})
        {
            ...
            {
                throw new Error('linkTos "weight", when provided, must be a finite number (relation "' + label + '" from "' + this.id + '" to "' + target.id + '")');
            }
            ...
            this._recordTrace('link_to',
            {
                targetId: target.id,
                label,
                weight: hasWeight ? options.weight : null,
                weighted: hasWeight,
                cardinality: constraint ? constraint.cardinality : null
            });
            return edge;
        }
        ...
        getCached(computeFn)
        {
            ...
            {
                this._recordTrace('content_cache_hit',
                {
                    contentHash: hash
                });
            ...
            this._recordTrace('content_cache_miss',
            {
                contentHash: hash
            });
            return result;
        ...
        linkToCached(target, label, computeFn, options = {})
        {
            ...
            {
                this._recordTrace('edge_cache_hit',
                {
                    targetId: target.id,
                    label
                });
            ...
            this._recordTrace('edge_cache_miss',
            {
                targetId: target.id,
                label
            });
...
`;
    ...
    // === CREATE WORKER FROM A REAL FILE, NOT A STRING/BLOB ===
    // Previously: `workerCode` was a template-literal string turned into a
    // SharedWorker via `new Blob([workerCode], {type: 'application/javascript'})`
    // + `URL.createObjectURL(blob)`. That blob: URL is regenerated every
    // session, so DevTools Overrides has no stable path to bind to, and the
    // worker's own source never shows up as real code in Sources — just a
    // synthetic VM entry with no breakpoints, no diffable file, nothing.
    // InjectorWorker.js now holds that same code as a real, stable-URL file
    // (loaded here as a sibling of BaseClassX.js/CPU.js/BIOS.js), so it can
    // actually be opened, edited, and overridden like any other script.
    const worker = new SharedWorker('./InjectorWorker.js');
    ...
    {
        const w = new SharedWorker('./InjectorWorker.js');
