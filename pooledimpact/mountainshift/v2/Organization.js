...
(function(root, factory) {
  ...
  class Organization extends BaseClassX {
    ...
    // ─── PROJECTIONS ──────────────────────────────────────────────
    // Coverage requirement: Night needs 2 staff assigned, Day needs a minimum of 1 \u2014
    // a date+shiftType combo is a real gap when fewer than that many shifts are filled
    // (status !== 'open' and staffId present), separate from any individual shift's own
    // isCoverageGap() flag (which only catches an explicitly-unfilled single shift).
    schedulingProjection(dateRange) {
      ...
      const MIN_STAFF = { day: 1, night: 2 };
      const filled = s => s.status !== 'open' && !!s.staffId;
      const byDateType = {};
      shifts.forEach(s => {
        const type = (s.shiftType || '').toLowerCase();
        const key = s.date + '|' + type;
        (byDateType[key] = byDateType[key] || []).push(s);
      });
      const understaffedGaps = [];
      Object.keys(byDateType).forEach(key => {
        const [date, type] = key.split('|');
        const min = MIN_STAFF[type];
        if (min == null) return;
        const filledCount = byDateType[key].filter(filled).length;
        if (filledCount < min) understaffedGaps.push({ date, shiftType: type, required: min, filled: filledCount });
      });
      const unfilledShiftGaps = shifts.filter(s => s.isCoverageGap ? s.isCoverageGap() : (s.status === 'open'));
      return {
        shifts,
        coverageGaps: unfilledShiftGaps,
        understaffedGaps,
        dayShiftCount: shifts.filter(s => (s.shiftType || '').toLowerCase() === 'day').length,
        nightShiftCount: shifts.filter(s => (s.shiftType || '').toLowerCase() === 'night').length
      };
    }
