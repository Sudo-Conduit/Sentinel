...
(function(root, factory)
{
    ...
    {
        root.Memory = factory(root.BaseClassX);
    }
}(typeof self !== 'undefined' ? self : this, function(BaseClassX)
{
    ...
    if (!BaseClassX)
    {
        throw new Error('Memory requires BaseClassX to be loaded first');
    }
    ...
    class Memory extends BaseClassX
    {
        ...
        static description = 'Volatile address-space model: schema-tracked region ledger over a runtime-held backing byte store.';
        static docs = ['Kernel-Machine-Architecture.md'];
        static tests = ['test/Memory.security.test.js', 'test/FullBootChain.lifecycle.test.js'];
         ...
         * (with none given) a fresh Uint8Array. Volatile: called again on
         * every reboot, previous contents are gone.
         *
         * `cpu && typeof cpu === 'object'`, NOT bare `cpu ? ... : ...` --
         * once this class is composed via ExtendX.extend(), attach() called
         * with NO argument (the documented "give me a fresh buffer" case)
         * does not get cpu === undefined: ExtendX's dispatcher always
         * appends its own next() callback as the trailing argument to every
         * dispatched call, landing in the omitted cpu slot. A bare truthy
         * check treats that injected FUNCTION as a real cpu and reads its
         * (nonexistent) .memory property, silently backing this instance
         * with undefined instead of a real buffer -- confirmed live,
         * breaking every subsequent read()/write()/alloc(). A real cpu
         * argument is always an object (typeof 'object'); the injected next
         * is always typeof 'function' -- same typeof-guard convention as
         * Kernel's ppid/memBytes/quantum and StructureMixin's label.
         * @param {Object} [cpu] - a live CPU.js instance to back this Memory
         *   with; omit for a fresh Uint8Array
         * @returns {Memory} this, for chaining
         */
        attach(cpu)
        {
        ...
        /**
         * @returns {Uint8Array} the live backing store
         * @throws {Error} if attach() has not been called yet
         */
        _store()
        {
            if (!_backing.has(this.id))
            {
                throw new Error('Memory.attach() must be called before use');
            }
         ...
         * -- alloc(pid, length) omitting label under composition would
         * otherwise store the injected next() function as region.label
         * instead of ''.
         * @param {number} pid - owning process id
         * @param {number} length - number of bytes to allocate
         * @param {string} [label] - human-readable tag for the region
         * @returns {Object} the allocated region { pid, base, length, label }
         * @throws {Error} if there is not enough room in the backing store
         */
        alloc(pid, length, label)
        {
            ...
            if (base + length > store.length)
            {
                throw new Error('Memory.alloc: out of memory (' + length + ' bytes requested at ' + base + ')');
            }
        ...
        /**
         * @param {number} pid - process whose regions should be released
         * @returns {Memory} this, for chaining
         */
        free(pid)
        {
        ...
        /**
         * @param {number} addr - starting byte offset
         * @param {number} length - number of bytes to read
         * @returns {Uint8Array} the requested slice of the backing store
         */
        read(addr, length)
        {
            const store = this._store();
        ...
        /**
         * @param {number} addr - starting byte offset
         * @param {Uint8Array|number[]} bytes - bytes to write
         * @returns {Memory} this, for chaining
         */
        write(addr, bytes)
        {
        ...
        /**
         * _backing is a Map (not a WeakMap) since the this.id fix above, so
         * it does not self-clean when an instance is garbage collected --
         * mirrors Physical.dispose()/Kernel.dispose()'s identical cleanup.
         */
        dispose()
        {
