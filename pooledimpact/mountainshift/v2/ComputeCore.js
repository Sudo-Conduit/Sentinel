 ...
 * @author Will Fobbs
 * @version 1.0.0
 * @description The Compute Processing Engine's internals -- providers, ops and
 *              composition. CPE.js is the opaque closure factory over this file;
 *              a caller goes through that, not here.
 *
 *              One `run(cmd)` surface over whatever
...
(function(root, factory)
{
    ...
    {
        root.ComputeCore = factory(root.ExtendX);
    }
}(typeof self !== 'undefined' ? self : this, function(ExtendX)
{
    ...
    class ComputeCore
    {
        constructor(opts)
        {
            ...
            if (!picked)
            {
                throw new Error(`ComputeCore: no provider "${opts.provider}" on this host`
        ...
        run(cmd)
        {
            ...
            {
                throw new Error('run(cmd): cmd needs an { op } string');
            }

            const handler = this['op_' + cmd.op];
            if (typeof handler !== 'function')
            {
                throw new Error(`run(): unknown op "${cmd.op}" -- have: `
                    + this.ops().join(', '));
            }
     ...
     * copying when it is not, so this still loads standalone in a browser that
    ...
    const API = {
        ...
        run(cmd, opts) { return API.create(opts).run(cmd); }
    };
