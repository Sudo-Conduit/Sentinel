 ...
 * @author Will Fobbs
 * @version 1.0.3
 * @description CPU.js — UMD + IIFE mechanical x86-shaped CPU emulator: registers, segment/
    ...
    function()
{
    ...
    class CPU
    {
        ...
        static description = 'Mechanical CPU Emulator';
        static docs = ['Kernel-Machine-Architecture.md'];
        static tests = ['test/CPU.security.test.js', 'test/FullBootChain.lifecycle.test.js'];

        /**
         * @param {Object} [options={}] - constructor options
         * @param {number} [options.memorySize=0x100000] - size (bytes) of the flat memory array
         */
        constructor(options)
        {
            options = options || {};
        ...
        #registerInstructions()
        {
            ...
            {
                self.EIP = self.memoryRead(self.ESP); self.ESP += 4;
            };
            ...
            {
                self.EFLAGS |= 0x200; self.interrupts.enabled = true; self.EIP += 1;
            };
            this.instructions['CLI'] = function()
            {
                self.EFLAGS &= ~0x200; self.interrupts.enabled = false; self.EIP += 1;
            };
            this.instructions['HLT'] = function()
            {
                self.config.halted = true; self.EIP += 1;
            };
            ...
            {
                self.config.caching = false; self.EIP += 1;
            };
        ...
        /**
         * Read a little-endian 32-bit value from the flat memory array.
         * @param {number} addr - byte address to read from
         * @returns {number} the 32-bit value at addr
         */
        memoryRead(addr)
        {
        ...
        /**
         * Write a little-endian 32-bit value into the flat memory array.
         * @param {number} addr - byte address to write to
        ...
        /**
         * Resolve a `[EXPR]`-style memory operand to a concrete address.
         * @param {string} expr - operand expression, e.g. "[EAX]" or "[0x1000]"
        ...
        /**
         * Recompute EFLAGS from an instruction result (zero/sign/overflow/parity bits).
         * @param {number} value - the result to derive flags from
         * @returns {void}
         */
        updateFlags(value)
        {
        ...
        /**
         * Push EFLAGS/CS/EIP and jump to the handler registered in the IDT (or a
         * default fallback vector if none is registered).
         * @param {number} intNum - interrupt vector number
         * @returns {void}
         */
        triggerInterrupt(intNum)
        {
            ...
            if (handler)
            {
                this.CS = handler.selector; this.EIP = handler.offset;
            }
            else
            {
                this.EIP = 0x0000F000;
        ...
        enableProtectedMode()
        {
            this.CR0 |= 0x1; this.config.protectedMode = true; this.config.realMode = false;
        }

        /** @returns {void} */
        enablePaging()
        {
            this.CR0 |= 0x80000000; this.config.paging = true;
        }

        /**
         * Fetch the opcode byte at the current EIP.
         * @returns {number} the opcode byte
         */
        fetch()
        {
        ...
        /**
         * Resolve an opcode byte to its registered instruction handler.
         * @param {number} opcode - opcode byte from fetch()
         * @returns {{name: string, fn: Function}|null} the matched instruction, or null
         */
        decode(opcode)
        {
        ...
        /**
         * Deterministic hash of an instruction mnemonic into an 8-bit opcode.
         * @param {string} name - instruction mnemonic, e.g. "MOV"
         * @returns {number} an 8-bit opcode value
         */
        ...
        /**
         * Invoke a named instruction handler with the given arguments.
         * @param {string} instruction - instruction mnemonic
         * @param {Array} args - arguments to apply to the handler
         * @returns {boolean} true if the instruction was found and executed
         */
        execute(instruction, args)
        {
            if (this.instructions[instruction])
            {
                this.instructions[instruction].apply(this, args); return true;
            }
            return false;
        }
         ...
         * @param {number} [steps=0] - number of instructions to run, or 0 to run until halted
         * @returns {number} the number of instructions executed
         */
        run(steps)
        {
            steps = steps || 0;
        ...
        /**
         * Read the two operand words following an instruction's opcode byte.
         * @param {number} eip - address just past the opcode byte
         * @returns {number[]} a two-element [dest, src]-shaped argument array
         */
        parseArgs(eip)
        {
        ...
        /** @param {Object} substrate - substrate state to attach @returns {void} */
        setSubstrate(substrate)
        {
            this.substrate = substrate;
        }

        /** @returns {Object} the current substrate state */
        getSubstrate()
        {
         ...
         * @returns {Object} registers, segments, control registers, config and halted/memoryUsed
         */
        dumpState()
        {
            return {
        ...
        /**
         * Reset all registers/segments/control registers/config to their
         * power-on defaults.
         * @returns {Object} the post-boot state, via dumpState()
         */
        boot()
        {
        ...
        /**
         * Alias for boot() -- a reset is the same power-on state transition.
         * @returns {Object} the post-reset state, via dumpState()
         */
        reset()
        {
            return this.boot();
