...
(function(root, factory) {
  ...
  function CPU(options) {
    ...
    for (var i = 0; i < memSize; i++) this.memory[i] = 0x00;
    ...
    for (var j = 0; j < 256; j++) this.IDT[j] = null;
  ...
  CPU.prototype.registerInstructions = function() {
    ...
    this.instructions['CMP'] = function(dest, src) {
      ...
      var result = self[dest] - srcValue; self.updateFlags(result); self.EIP += 3;
    };
    ...
    this.instructions['LIDT'] = function(addr) {
      var base = self.memoryRead(addr); var limit = self.memoryRead(addr + 2);
  ...
  CPU.prototype.memoryRead = function(addr) {
    var b0 = this.memory[addr] || 0, b1 = this.memory[addr + 1] || 0, b2 = this.memory[addr + 2] || 0, b3 = this.memory[addr + 3] || 0;
  ...
  CPU.prototype.disableCache = function() { this.config.caching = false; };
  CPU.prototype.enableProtectedMode = function() { this.CR0 |= 0x1; this.config.protectedMode = true; this.config.realMode = false; };
  CPU.prototype.enablePaging = function() { this.CR0 |= 0x80000000; this.config.paging = true; };
  CPU.prototype.fetch = function() { return this.memoryRead(this.EIP); };
  ...
  CPU.prototype.setSubstrate = function(substrate) { this.substrate = substrate; };
  CPU.prototype.getSubstrate = function() { return this.substrate; };
  ...
  CPU.prototype._toHex32 = function(value) { var hex = value.toString(16); while (hex.length < 8) hex = '0' + hex; return hex; };
  CPU.prototype._toHex16 = function(value) { var hex = value.toString(16); while (hex.length < 4) hex = '0' + hex; return hex; };
  ...
  CPU.prototype.reset = function() { return this.boot(); };
