...
(function(root, factory)
{
    ...
    {
        define(['./BaseClassX.js', './Environment.js', './Kernel.js'], factory);
    }
    else if (typeof module === 'object' && module.exports)
    {
        module.exports = factory(require('./BaseClassX.js'), require('./Environment.js'), require('./Kernel.js'));
    }
    else
    {
        root.BIOS = factory(root.BaseClassX, root.Environment, root.Kernel);
    }
}(typeof self !== 'undefined' ? self : this, function(BaseClassX, Environment, Kernel)
{
    ...
    if (!BaseClassX)
    {
        throw new Error('BIOS requires BaseClassX to be loaded first');
    }

    const _kernelFactories = new Map(); // this.id -> factory({bootedFrom, firmwareType, cores}) | null
    function defaultKernelFactory(opts)
    {
        return new Kernel(opts);
    }

    /**
     * NOT `typeof this.addChild === 'function'` -- BaseClassX itself
     * already defines a NATIVE addChild(childInstance) for its own
     * always-on tree (a totally separate feature from StructureMixin's,
     * taking a real BaseClassX instance, not an extId string). That native
     * method exists on every BIOS instance whether or not StructureMixin
     * is composed, so a bare typeof check finds it every time and calls it
     * with the wrong argument shape -- proven live: "Child must be an
     * instance of BaseClassX" thrown from BaseClassX.js, not from anything
     * obviously BIOS-related. This checks for a graph/both-mode
     * StructureMixin SPECIFICALLY, by mixinId prefix, so the native
     * BaseClassX tree is never confused with it.
     * @param {Object} instance - a BaseClassX (sub)class instance to inspect
     * @returns {boolean} true if instance is composed with a graph/both-mode StructureMixin
     */
    ...
    class BIOS extends BaseClassX
    {
         ...
         * Registry is attached, its 'bootDeviceOrder'/'firmwareType'
         * entries (if present) override this instance's own schema
         * defaults for the boot that follows.
         * @param {Object} registry - anything shaped like a Registry (get/has)
         * @returns {BIOS} this, for chaining
         */
        attachRegistry(registry)
        {
            this.registryRef = registry;
            if (registry && registry.has && registry.has('bootDeviceOrder'))
            {
                this.bootDeviceOrder = registry.get('bootDeviceOrder');
            }
            if (registry && registry.has && registry.has('firmwareType'))
            {
                this.firmwareType = registry.get('firmwareType');
            }
        ...
        /**
         * fs is optional: anything exposing findBootEntry(device,
         * firmwareType) (sync or returning a Promise). Without one, and for
         * the 'disk' step specifically, falls back to BootDeviceScan
         * (IDB/OPFS/Cache, by the project's meshui-vol- naming convention)
         * when it's loaded globally.
         *
         * iso is optional: an ISO instance (step 6-8) to install and boot
         * from when nothing bootable is found anywhere — without one,
         * boot() resolves bootedFrom:'none', matching real hardware's "no
         * bootable device" outcome.
         * @param {Object} physical - the Physical to POST and run the boot on
         * @param {Object} [fs] - filesystem-shaped boot-entry lookup (findBootEntry)
         * @param {Object} [iso] - an ISO to install/boot from as a last resort
         * @returns {Promise<Kernel>} the freshly booted, attached Kernel
         */
        async boot(physical, fs, iso)
        {
            ...
            if (hasGraphStructure(this) && hasGraphStructure(kernel))
            {
                this.addChild(kernel._extId);
            }
            return kernel;
        }
    }
