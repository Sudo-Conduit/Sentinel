...
}(typeof self !== 'undefined' ? self : this, function()
{
    ...
    {
        if (name.startsWith(USB_PREFIX))
        {
            return { id: name.slice(USB_PREFIX.length), removable: true };
        }
        if (name.startsWith(VOLUME_PREFIX))
        {
            return { id: name.slice(VOLUME_PREFIX.length), removable: false };
        }
        return null;
    }

    /**
     * Scan the IndexedDB surface for meshui-vol-/meshui-usb- entries.
     * FileFsX's idb backend keeps every mount as a ROW in one shared
     * 'FileFsX' database's 'mounts' object store (keyed by mount key) —
     * not as a separate database per mount, so this scans keys in that
     * store, not indexedDB.databases() (which would only ever find literal
     * database names, never individual FileFsX mount keys).
     * @returns {Promise<Array<{surface: string, id: string, removable: boolean}>>} matching hits on this surface
     */
    ...
    /**
     * Scan the OPFS surface for meshui-vol-/meshui-usb- entries.
     * FileFsX's opfs backend stores a mount as a FILE named '<key>.json'
     * at the OPFS root, not a directory named '<key>' — same class of
     * convention mismatch scanIDB() had.
     * @returns {Promise<Array<{surface: string, id: string, removable: boolean}>>} matching hits on this surface
     */
    async function scanOPFS()
    {
        ...
        try
        {
            ...
            for await (const [name, handle] of root.entries())
            {
                ...
                if (c !== null)
                {
                    found.push({ surface: 'opfs', id: c.id, removable: c.removable });
                }
            }
            return found;
        }
    ...
    /**
     * Scan the Cache API surface for meshui-vol-/meshui-usb- entries.
     * @returns {Promise<Array<{surface: string, id: string, removable: boolean}>>} matching hits on this surface
     */
     ...
     * @returns {Promise<Array<{surface: string, id: string, removable: boolean}>>} every hit found across all surfaces
     */
    ...
    const BootDeviceScan = { VOLUME_PREFIX, USB_PREFIX, scanIDB, scanOPFS, scanCache, scanAll };

    BootDeviceScan.name = 'BootDeviceScan';
    BootDeviceScan.author = 'Will Fobbs';
    BootDeviceScan.version = '1.0.0';
    BootDeviceScan.description = 'Step 5 of the boot sequence: scans IndexedDB, OPFS, and the Cache API for bootable volumes matching the project\'s naming convention.';
    BootDeviceScan.docs = ['Kernel-Machine-Architecture.md'];
    BootDeviceScan.tests = ['test/NextInjection.audit.test.js'];

    return BootDeviceScan;
}));
