...
// all), ISO (only zero-arg instance methods), Installer/BootDeviceScan
// (never reachable through ExtendX's dispatcher at all -- statics and a
// plain object are structurally immune regardless of composition),
// FileFsBootAdapter (its one method has no optional trailing param for
// next to land in). Registry has one real, currently-unreachable gap in
// set() -- proven here, and documented in Registry.js itself as a known
// limitation rather than a false fix, since value's legitimate type space
// (anything) makes the typeof-guard trick used elsewhere inapplicable.
//
...
async function run() {
    ...
    check('ISO: composed instance methods take no arguments -- next() injection has nowhere to land', () => {
