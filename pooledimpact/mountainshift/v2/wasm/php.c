/*
 * php.c — WASM module: run a real `php` process, on a whitelist,
 ...
 * once it's done." Nothing about WHAT php.js does is interpreted
 * here; this file only ever decides that the program to run is
 * literally "php" (hardcoded, never taken from argv) and builds the
 ...
 * "php", the host is still the one that decides whether to honor
 ...
 *         -Wl,--export-memory -o php.wasm php.c
 *
 ...
 *   cmdline: php [any real php argv, quote-aware -- e.g.
 *            php -r 'echo 1+1;']
 ...
 *   SPAWN\0php\0argc\0(arg\0){argc}stdinlen\0<stdinlen raw bytes>
 ...
 * --eval 'echo 1+1;' has to survive as one argv entry, and
...
i32 run(i32 ptr, i32 len) {
    ...
    if (!exec_response) {
        ...
        /* argv[0] is "php" itself -- never forwarded as an arg to the
         * real php binary, same as a shell never passes argv[0] of
        ...
        write_str(out, &out_len, output_cap, "php");
        out[out_len++] = '\0';
    ...
    /* Phase 2: the host already ran the real php process. Parse its
