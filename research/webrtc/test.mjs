...
const bDone = B.page.evaluate(async ({ WS, POPCNT_SEED, N }) => {
  ...
  const t = new window.WebRTCTransport({ url: WS, room: 'r1', peer: 'B', initiator: false, iceServers: [] });
...
const aInfo = await A.page.evaluate(async ({ WS, POPCNT_SEED, N, MSG }) => {
  ...
  const t = new window.WebRTCTransport({ url: WS, room: 'r1', peer: 'A', initiator: true, iceServers: [] });
  const info = await t.connect();
  t.send(enc.value);
