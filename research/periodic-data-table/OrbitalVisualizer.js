(function(root) {
    ...
    var OrbitalVisualizer = function(container, symbol, orbitalChoice, options) {
        ...
        this.target = { x: 0, y: 0, z: 0 }; // orbit/look-at center; Cmd+Drag pans this
    ...
    OrbitalVisualizer.prototype._setupCameraControls = function() {
        ...
        this.renderer.domElement.addEventListener('mousemove', function(e) {
            ...
            if (e.metaKey) {
                // Cmd only, not Ctrl: Ctrl+Drag collides with OS/browser
                // gestures (e.g. trackpad zoom, right-click emulation) on
                // several platforms, which made panning unreliable there.
                self._pan(dx, dy);
            } else {
        ...
        // Touch stays single-finger-orbit only; Cmd+Drag pan is a desktop
    ...
    // The renderer/camera are only ever sized once, at construction, from
    // container.clientWidth/Height — so when the *container's* CSS size
    // changes later (a host page's layout reflowing, e.g. Orbital Scope's
    // grid going full-width once only one scope is left), the canvas never
    // follows unless something tells it to. A ResizeObserver on the
    // container is exactly that signal.
    OrbitalVisualizer.prototype._setupResize = function() {
        var self = this;
    ...
    OrbitalVisualizer.prototype._onResize = function() {
    ...
    // Undoes any Cmd+Drag panning and returns to the default framing.
